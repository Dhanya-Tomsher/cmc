...
ALTER TABLE `services` ADD `category_id` INT NULL DEFAULT NULL AFTER `id`;

ALTER TABLE `services` ADD FOREIGN KEY (`category_id`) REFERENCES `pricelist_categories`(`id`) ON DELETE CASCADE ON UPDATE NO ACTION;

ALTER TABLE `vets` ADD `last_schedule_from` TIME NULL DEFAULT NULL AFTER `shift_to`, ADD `last_schedule_to` TIME NULL DEFAULT NULL AFTER `last_schedule_from`;

ALTER TABLE `dynamic_invoice_details` ADD `category_id` INT NULL DEFAULT NULL AFTER `dynamic_invoice_id`;

ALTER TABLE `dynamic_invoice_details` ADD FOREIGN KEY (`category_id`) REFERENCES `pricelist_categories`(`id`) ON DELETE CASCADE ON UPDATE SET NULL;